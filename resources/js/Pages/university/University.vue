                ...
                <v-menu>
                    ...
                    <v-list>
                        <v-list-item>
                            ...
                            <v-list-item-title
                                @click="view(item)"
                                class="cursor-pointer d-flex gap-3 pt-2"
                            >
...
<script setup>
import UniversityDetailsDialog from "./UniversityDetailsDialog.vue";
...
const view = async (item) => {
    try {
        showDetailsDialog.value = true; // Show dialog first
        const data = await UniversityRepository.FetchUniversity(item.id);
        selectedUniversity.value = data;
    } catch (err) {
        console.error("Error loading university details", err);
        showDetailsDialog.value = false; // Hide dialog if error
    }
};
</script>
